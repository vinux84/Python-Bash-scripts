...
def mod_ping(ip_addr, switch_status, camera_amount):
    ...
    for cam_num in range(1, camera_amount + 1):
        device_dict[f'Camera {cam_num}'] = cam_num
...
def intro():
    ...
    while True:
        ...
        if second_switch.lower() == "y":
            one_switch = "no"
            break
        elif second_switch.lower() == "n":
            one_switch = "yes"
            break
        else:
            print("Enter y or n")
    while True:
        ...
        if which_node.lower() == "y":
            device_dict['NODE'] = '102'
            break
        elif which_node.lower() == "n":
            break
        else:
            print("Enter y or n")
            
    while True:
        camera_amount = input("\nPlease enter number of cameras: ")
        if camera_amount.isnumeric():
            camera_int = int(camera_amount)
            break
        else:
            print("Enter only numbers")
        
    mod_ping(add_subnet, one_switch, camera_int)
